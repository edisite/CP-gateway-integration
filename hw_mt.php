...
// block mt huawai
/*
echo '<?xml version="1.0"?><push><transid>'.$in_trxids.'</transid><status>10</status><msg>Blok MT</msg></push>';
log_write($in_trxids, "  error", "Blok MT");
log_write($in_trxids, "finish", "counted as error\m");
return;
*/	
...
if($in_waptype == "1") // wap message, since 2011-12-14 WAP will be treated as SMS, change # to 1 to treat wap as wap
{
	...
	if ($pos_http > 5)
	{
		...
		if (strlen($response) > 130) { $response = substr($response, 0, 150)."..."; }
	}
	else
	{
...
else
{
	...
	$HITMTD = "get";
	$URL = "";
	$prefix = "http://202.152.162.163:8329";
