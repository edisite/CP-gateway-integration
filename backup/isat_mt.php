...
if(strtolower($in_keywod) == "artispull")
{
	$in_keywod = "artis";	
}
if(strtolower($in_keywod) == "jpoppull")
{
	$in_keywod = "jpop";	
}
...
if (DO_CHECK_PARAM)
{
	...
	$LogParam = ": ".$partnerID." : ".$in_keywod." : ".$in_srvcid." : ".$in_msisdn." : ".$in_dlvmtd." : ".$in_trxids." : ".$in_shcode." : ".$in_age;
...
// check if incoming MT will generate Free Talk service, if it does, then store it into free talk table
	...
	if (!$dbCN)
	{
		log_write($in_operat, $cpid, $in_trxids, "  error", "FATAL ERROR ##############################");
		log_write($in_operat, $cpid, $in_trxids, "  error", "database connection failed");
		log_write($in_operat, $cpid, $in_trxids, "  solution", "mt will be processed, but freetalk will not");
		log_write($in_operat, $cpid, $in_trxids, "  solution", "contact developer to analyze this problem");
	}
	else
	{
		...
		{
			log_write($in_operat, $cpid, $in_trxids, "  error", "FATAL ERROR ##############################");
			log_write($in_operat, $cpid, $in_trxids, "  error", "open database failed");
			log_write($in_operat, $cpid, $in_trxids, "  solution", "mt will be processed, but freetalk will not");
			log_write($in_operat, $cpid, $in_trxids, "  solution", "contact developer to analyze this problem");
		}
	}	
	...
	$SQLCEKBLACK = "SELECT * FROM blacklist_msisdn._msisdn WHERE msisdn = '".$in_msisdn."' AND (sc = 'all' OR sc = '9155')";
	$QRYCEKBLACK = mysql_num_rows(mysql_query($SQLCEKBLACK));
	
	if($QRYCEKBLACK >= 1)
	{
		log_write($in_operat, $cpid, $in_trxids, "  error", $SQLCEKBLACK);
		log_write($in_operat, $cpid, $in_trxids, "  error", "MSISDN BLACKLIST");
		log_write($in_operat, $cpid, $in_trxids, "  error", "Transaction Not sent for Operator");
	...
	if($in_dlvmtd == "pull")
	{
		...
		if($NUMCEK >= 1)
		{
			...
			log_write($in_operat, $cpid, $in_trxids, "  Set Queue Retry", $SQLQueue);
		}
	}
