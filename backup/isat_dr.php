...
else
{
	...
	{			
		...
		//createLogDR($tid, "Set cek dc ", $SQLCEK);
	...
	if (HAVING_FREETALK)
	{
		...
		if ($in_drstat == DR_STATUS_OK && ($in_servid == SIDDR2 or $in_servid == SIDDR204) && $dosend > 0)
		{
			...
			if ($dosend == 2) // send freetalk
			{
				...
				$URL = $URL."&sms=".rawurlencode("Horee, Kamu mendapat gratis bicara 60menit dari Program 6768 Indosat. Untuk cek gratis bicaramu ketik *555*2# kemudian OK/Yes/Dial");
