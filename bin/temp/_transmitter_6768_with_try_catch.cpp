...
int cast_str2int(string sParam)
{
	...
	try
	{
		irc = atoi(sParam.c_str());
		return irc;
	}
	catch (int e)
	{
		cout << "UNEXPECTED ERROR: cast_str2int" << endl;
...
string cast_int2str(int iParam)
{
	...
	{
		cout << "UNEXPECTED ERROR: cast_int2str" << endl;
...
string cast_str2lower(string sParam)
{
	...
	{
		cout << "UNEXPECTED ERROR: cast_str2lower" << endl;
		cout << "    sParam = [" << sParam << "]" << endl;
		exit(0);
	}
}
...
string cast_str2upper(string sParam)
{
	...
	{
		cout << "UNEXPECTED ERROR: cast_str2upper" << endl;
		cout << "    sParam = [" << sParam << "]" << endl;
		exit(0);
	}
}
...
bool cast_str2bool(string sParam)
{
	...
	{
		cout << "UNEXPECTED ERROR: cast_str2bool" << endl;
		cout << "    sParam = [" << sParam << "]" << endl;
		exit(0);
	}
}
...
string trimTABCRLF(string sParam)
{
	...
	{
		cout << "UNEXPECTED ERROR: trimTABCRLF" << endl;
		cout << "    sParam = [" << sParam << "]" << endl;
		exit(0);
	}
}
...
string cast_strquote(string sParam)
{
	...
	{
		cout << "UNEXPECTED ERROR: cast_strquote" << endl;
		cout << "    sParam = [" << sParam << "]" << endl;
		exit(0);
	}
}
...
string xml_read_file(char *cFilename)
{
	...
	{
		cout << "UNEXPECTED ERROR: xml_read_file" << endl;
...
string xml_get_value(string sSource, string sKey)
{
	...
	{
		cout << "UNEXPECTED ERROR: xml_get_value" << endl;
...
void log_write(string spThread, string spTrxID, string spHead, string spMsg)
{
	...
	{
		cout << "UNEXPECTED ERROR: log_write" << endl;
		cout << "    spThread = [" << spThread << "]" << endl;
		cout << "    spTrxID = [" << spTrxID << "]" << endl;
		cout << "    spHead = [" << spHead << "]" << endl;
		cout << "    spMsg = [" << spMsg << "]" << endl;
		exit(0);
	}
}
...
int db_connect(int index)
{
	try
	{
		...
		if (index == 1)
		{
			...
			try { mysql_close(gdb1CN); } catch(int ee) {}
		...
		{
			// initialize database connection structure
			try { mysql_close(gdb2CN); } catch(int ee) {}
	...
	{
		cout << "UNEXPECTED ERROR: db_connect" << endl;
...
int db_query(int index, string sSQL)
{
	...
	{
		cout << "UNEXPECTED ERROR: db_query" << endl;
		cout << "    index = [" << index << "]" << endl;
		cout << "    sSQL = [" << sSQL << "]" << endl;
		exit(0);
	}
...
void *thread_transmitter(void *arg)
{
		...
		while (gTerminateApplication == false)
		{
			...
			if (GDO_SEND_URL)
			{
				...
				if (sMTD == "post" || sMTD == "1")
				{
					...
					try { sTmp = sURL.substr(0, 7); } catch (int ee) { sTmp = ""; }
	...
	{
		cout << "UNEXPECTED ERROR: thread_transmitter" << endl;
		cout << "    sSQL = [" << sSQL << "]" << endl;
		exit(0);
	}
}
...
void *thread_updater(void *arg)
{
	...
	{
		cout << "UNEXPECTED ERROR: thread_updater" << endl;
		cout << "    sTRXID = [" << sTRXID << "]" << endl;
		cout << "    sSQL = [" << sSQL << "]" << endl;
		exit(0);
	}
}
...
void *socket_open(void *arg)
{
	...
	{
		cout << "UNEXPECTED ERROR: socket_open" << endl;
		cout << "    giiniportno = [" << giiniportno << "]" << endl;
		exit(0);
	}
}
...
bool socket_close()
{
	...
	{
		cout << "UNEXPECTED ERROR: socket_close" << endl;
		cout << "    giiniportno = [" << giiniportno << "]" << endl;
		exit(0);
	}
}
...
void setting_get_keywords(string psKeywords, string psURLTransmit, string psURLResponse)
{
	...
	{
		cout << "UNEXPECTED ERROR: setting_get_keywords" << endl;
		cout << "    psKeywords = [" << psKeywords << "]" << endl;
		cout << "    psURLTransmit = [" << psURLTransmit << "]" << endl;
		cout << "    psURLResponse = [" << psURLResponse << "]" << endl;
		exit(0);
	}
}
...
void setting_get_values(string sText)
{
	...
	{
		cout << "UNEXPECTED ERROR: setting_get_values" << endl;
...
void setting_show_values()
{
	...
	{
		cout << "UNEXPECTED ERROR: setting_show_values" << endl;
		exit(0);
	}
}
...
void setting_log_values(string sPathSetting)
{
	...
	{
		cout << "UNEXPECTED ERROR: setting_log_values" << endl;
		cout << "    sPathSetting = [" << sPathSetting << "]" << endl;
		exit(0);
	}
}
...
int main (int argc, char **argv)
{
	try
	{
			...
			if (gquePULL.count() < 1)
			{
				...
				for (i = 0; i < giinipull_sql_cnt; i++)
				{
					...
					else
					{
						...
						try { mysql_free_result(tdbRS); } catch (int ee) {}
		...
		try { socket_close(); } catch (int ee) {}
		try { mysql_close(gdb1CN); } catch (int ee) {}
		try { mysql_close(gdb2CN); } catch (int ee) {}
		try { mysql_library_end(); } catch (int ee) {}
		try { memset(gthinfoPULL, 0, sizeof(gthinfoPULL)); } catch (int ee) {}
		try { memset(gthinfoPUSH, 0, sizeof(gthinfoPUSH)); } catch (int ee) {}
	}
	catch (int e)
	{
		cout << "UNEXPECTED ERROR: main" << endl;
		exit(0);
	}
}