...
const string	GSFILELASTALIVE = "f2m_last_alive.nfo";		// monitoring, file name where application store it's last breath
...
int db_query_n_evaluate(int index, string sSQL, bool &result)
{
	/*
	return values
		return -1, 0, 1
		...
		while ((aFile = readdir(aDir)) != NULL)
		{
			...
			if (sTemp != "")
			{
				...
				if (iCNN > 0 && sSQL != "")
				{
					...
					// execute query, if failed then write log then exit application
					isTrue = false;
					n = db_query_n_evaluate(iCNN, sSQL, isTrue);
					if (n < 0)
					{
						log_write(sinTrxID, "  error", "query failed");
						log_write(sinTrxID, "  solution", "move file into trash directory [" + file_trash(gsinidirsrc + cFilename, cFilename) + "]");
