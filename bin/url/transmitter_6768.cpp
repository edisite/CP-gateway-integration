...
void *thread_transmitter(void *arg)
{
	...
	while (gTerminateApplication == false)
	{
		...
		// get partner address, method of delivery and send timeout
		gcolKwTransmit.item(sKeyword, s1, sMTD, s3, sTemp);
		iTOUT = cast_str2int(sTemp); // convert partner sending timeout to integer
		...
		if (GDO_SEND_URL)
		{
			...
			if (sMTD == "post" || sMTD == "1")
			{
				...
				try { sTemp = sURL.substr(0, 7); } catch (int e) { sTemp = ""; }
				...
				if (sTemp == "http://")
				{
					...
					{
						sTemp = sURL;
						sURL = sTemp.substr(0, int(t));

						try { sDATA = sTemp.substr(int(t) + 1); }
						catch (int e)
						{
				...
				{
					n = objURL.send_post(sURL, sDATA, sURLResponse, giiniURLTimeout);
			...
			else // SEND WITH HTTP GET METHOD ####################################################################################
			{
				...
				for (i = 0; i < GIMAXTRYSENDURL; i++)
				{
					n = objURL.send_get(sURL, sURLResponse, giiniURLTimeout);
					if (n == 0 && sURLResponse != "") { break; }
					usleep(giinisleep2);
				}
				if (n == 0)
		...
		if (GDO_PROCESS_RESPONSE)
		{
			...
			else
			{
				...
				if (gbinifwdenable)
				{
					...
					for (i = 0; i < GIMAXTRYSENDURL; i++)
					{
						m = objURL.send_get(sURL, sFwdResponse, giiniURLTimeout);
...
void setting_show_values()
{
	...
	while (1)
	{
		if (gcolKwTransmit.read(sVal1, sVal2, sVal3, sVal4) == true)
		{
			i = i + 1;
	...
	while (1)
	{
		if (gcolKwResponse.read(sVal1, sVal2, sVal3, sVal4) == true)
		{
			i = i + 1;
			cout << "    " << i << ": " << sVal1 << ": " << sVal2 << ": " << sVal3 << ": " << sVal4 << endl;
		}
