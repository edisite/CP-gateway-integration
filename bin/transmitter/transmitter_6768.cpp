...
bool	gsiniforward2used = false;	// App setting variable, if transmit's response is a dr instead of success status then set this to yes
string	gsiniforward2name = "";		// so that delivery report received in response body will be forwarded to this receiver
string	gsiniforward2addr = "";		// which when forwarding should be forwarded to this url address
string	gsiniforward2mtod = "";		// url method when forwarding received response
	...
	while (gTerminateApplication == false)
	{
		...
		if (GDO_GET_QUEUE)
		{
			...
			pthread_mutex_lock(&tmutex);
				if (thinfo.type == "pull")
				{
					n = gquePULL.get(sTHR, sDNA, sTRXID, sMSISDN, sSID, sKeyword, sMTD, sURL);
				}
				else
				{
					n = gquePUSH.get(sTHR, sDNA, sTRXID, sMSISDN, sSID, sKeyword, sMTD, sURL);
				}
			pthread_mutex_unlock(&tmutex);

			if (n != 0 || sDNA == "" || sTRXID == "" || sURL == "")
			{
				usleep(giinisleep1);
				goto posNextQueue;
			}
		...
		if (GDO_SEND_URL)
		{
			...
			if (sMTD == "post" || sMTD == "1")
			{
				...
				else // if url address not found
				{
					...
					sTmp = gcolKeyword.getURL(sKeyword);		// get url address based on MO keyword
					log_write(thinfo.name, sTRXID, "  post", "url found by keyword [" + sTmp + "]");

					if (sTmp == "") // if url address not found then set to default url
					{
			...
			else // SEND WITH HTTP GET METHOD ####################################################################################
			{
				...
				else
				{
					...
					if (sTmp == "")							// if url address not found then set to default url
					{ 
						log_write(thinfo.name, sTRXID, "  get", "url not found for keyword [" + sKeyword + "], getting default url");
						sTmp = gsinidefaulturl; 
					}		
					sURL = sTmp + sURL;						// set a complete url with its parameters
				}
		...
		if (GDO_PROCESS_RESPONSE)
		{
			...
			else
			{
				...
				if (gsiniforward2used)
				{
					if (gsiniforward2mtod == "post")
					{
						sURL = gsiniforward2addr + "?trxid=" + sTRXID + "&msisdn=" + sMSISDN + "&serviceid=" + sSID + "&raw=" + curl_easy_escape(NULL, sURLResponse.c_str(), 0);
					}
					else
					{
						t = sURL.find("?");
...
void *thread_updater(void *arg)
{
	...
	{
		// get queue
		n = gqueEXEC.get(sTHR, sLOG, sTRXID, sSQL);
		if (n != 0) { usleep(giinisleep2); goto posNextQueue; }
		log_write(sTHR, sTRXID, "  execute", sSQL);

		// execute query retrieved from queue
		if (db_query(2, sSQL) == 0)
		{
...
void setting_get_keywords(string sOwnerURL, string sOwnerKeyword)
{
	int i;
	int n;
	...
	for (i = 0; i < n; i++)
	{
		...
		if (sOwnerKeyword.substr(i, 2) == "</")
		{
			...
			else if (iMode == 2)
			{
				...
				sURL = xml_get_value(sOwnerURL, sOwner);
				gcolKeyword.put(sOwner, sKeyword, sURL);
				sKeyword = "";
			}
		...
		{
			s = sOwnerKeyword.substr(i, 1);
			if (s == " ") // invalid characters and will not be processed
			{
				// do nothing
			}
			else if (s == "<") // if opening tag found, switch to recording owner mode
			{
				iMode = 1;
				sOwner = "";
			}
			else if (s == ">")
			{
				...
				else if (iMode == 1) // if closing tag found while in recording owner, then switch it to record keyword mode
				{
					iMode = 2;
				}
			}
...
void setting_get_values(string sText)
{
	...
	sTemp1 = xml_get_value(sText, "db_queue");
		gsinidb1addr = xml_get_value(sText, "host");
		gsinidb1name = xml_get_value(sText, "database");
		gsinidb1user = xml_get_value(sText, "user");
		gsinidb1pswd = xml_get_value(sText, "password");
		gsinidb1tabl = xml_get_value(sText, "table");
	...
	sTemp1 = xml_get_value(sText, "success_response");
		giiniresok_cnt = 0;
		for (i = 0; i < GIMAXOKRESPONSE; i++)
		{
			sSQL = xml_get_value(sTemp1, "ok" + cast_int2str(i + 1));
			if (sSQL == "") { break; }
			giiniresok_cnt = giiniresok_cnt + 1;
			gsiniresok[giiniresok_cnt] = cast_str2upper(sSQL);
		}

	sTemp1 = xml_get_value(sText, "owners_url");
	sTemp2 = xml_get_value(sText, "keywords_owner");
		setting_get_keywords(sTemp1, sTemp2);
		gsinidefaulturl = xml_get_value(sTemp1, "default");

	sTemp1 = xml_get_value(sText, "dr_received_as_response_then_forward_to");
		sTemp2 = xml_get_value(sTemp1, "enabled"); transform(sTemp2.begin(), sTemp2.end(), sTemp2.begin(), ::tolower);
		gsiniforward2used = false;
		if (sTemp2 == "1") { gsiniforward2used = true; }
		if (sTemp2 == "on") { gsiniforward2used = true; }
		if (sTemp2 == "yes") { gsiniforward2used = true; }
		if (sTemp2 == "true") { gsiniforward2used = true; }

		gsiniforward2name = xml_get_value(sTemp1, "forward_to_name");
		gsiniforward2addr = xml_get_value(sTemp1, "forward_to_url");
		gsiniforward2mtod = cast_str2lower(xml_get_value(sTemp1, "forward_method"));
}
...
int main (int argc, char **argv)
{
	...
	cout << "keywords mapping" << endl;
	cout << "number; owner; keyword; url" << endl;
	cout << "===================================================================" << endl;
	i = 0;
	gcolKeyword.first();
	while (gcolKeyword.next() == 0)
	{
		i = i + 1;
		gcolKeyword.read(sVal1, sVal2, sVal3);
		cout << "    " << i << ": " << sVal1 << ": " << sVal2 << ": " << sVal3 << endl;
	}
	...
	while (gTerminateApplication == false)
	{
		...
		{
			if (difftime(time(NULL), 	[i].check) > giinitimeoutpush)
			{
				// terminate previous thread first
