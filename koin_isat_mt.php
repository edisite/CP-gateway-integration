...
if (DO_SET_SDMCODE)
{
	...
	$sdmcode["spab"] = "spab_6768";//20140703
...
// check if incoming MT will generate Free Talk service, if it does, then store it into free talk table
	...
	$SQLCEKBLACK = "SELECT * FROM blacklist_msisdn._msisdn WHERE msisdn like '%".$in_msisdn."%' limit 1";
	$QRYCEKBLACK = mysql_query($SQLCEKBLACK);
	$NUMCEKBLACK = mysql_num_rows($QRYCEKBLACK);
...
$URL = "";
$prefix = "";
if(($in_keywod == "caper") || ($in_keywod == "fun5"))
{
	if($in_keywod == "caper")
	{
		$URL = $prefix."?uid=pulsa96768";
		$URL = $URL."&pwd=pulsapass";
	}
	else
	{
		$URL = $prefix."?uid=pulsa96869";
		$URL = $URL."&pwd=pulsapass";
	}
}
else
