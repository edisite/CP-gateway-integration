...
for($x=0;$x<=$beda[beda];$x++)
{
	...
	$q = mysql_query("SELECT date_format(insertime,'%d-%m-%Y') as date,keyword, sid, count(msisdn) as dr2 FROM `report_dr_2` where devmethod='push' and keyword='$keyword' and insertime like '$bb[cc] %' group by date,keyword,sid order by date,sid");
					
	while($result=mysql_fetch_object($q))
	{				
		if($result->sid=='46500112040001') {$price = 1;}
		else if($result->sid=='46500112040003') {$price = 1000;}
		else if($result->sid=='46500112138006') {$price = 5000;}
		else if($result->sid=='46500112134007') {$price = 5000;}
		else if($result->sid=='46500112042002') {$price = 1;}
		else if($result->sid=='46500112042004') {$price = 1300;}
		else if($result->sid=='46500112042005') {$price = 2000;}
		else if($result->sid=='46500112020008') {$price = 2800;}
		else if($result->sid=='46500112034009') {$price = 0;}
		else if($result->sid=='46500112034010') {$price = 2800;}
