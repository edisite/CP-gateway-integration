...
<form name="form1" method="post" action="pullreport.php">
...
<td>Date : <?=$from?> s/d <?=$to?> | keyword : <?=$keyword?> | <a href="../print/6768/pull.php?from=<?=$from?>&to=<?=$to?>&keyword=<?=$keyword?>&cp=KOIN_6768&telco=isat&method=PULL"><img src="http://122.129.112.169/portal/report/img/pdf.png" height="20px" /></a></td>
</tr>
<tr class="shade" onmouseover="this.className = &#39;rowselected&#39;;" onmouseout="this.className = &#39;shade&#39;;" valign="top">
<td>
	<table border="1" style="border-collapse:collapse;align:center;">
		...
		<?
			...
			for($x=0;$x<=$beda[beda];$x++)
			{
				...
				$q = mysql_query("SELECT date_format(insertime,'%d-%m-%Y') as date,keyword, sid, count(msisdn) as dr2 FROM `report_dr_2` where devmethod='pull' and keyword like '$keyword%' and insertime like '$bb[cc] %' group by date,keyword,sid order by date,sid");
					...
					if($result->dr2 != 0) {
						echo "
							<tr>
								...
								<td align='center'>$result->keyword</td>
		 ...
		 $qq = mysql_query("select sid,max(insertime) as maxdr from report_dr_2 where keyword like '$keyword%' and devmethod='pull'");
$ss = mysql_fetch_object($qq);
echo "<tr><td colspan=6>$ss->maxdr</td><td>$ss->sid</td></tr>";
			$db->free_memory($q);
			$db->close($con);
		?>
	<table>
