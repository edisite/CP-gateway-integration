...
// store source address (if exist) into database
unset($arrf, $arrv);
$arrf[0] = "trxid";			$arrv[0] = "'".$in_trxids."'";
$arrf[1] = "dtm";			$arrv[1] = "NOW()";
$arrf[2] = "tied_name";		$arrv[2] = "'ip'";
$arrf[3] = "tied_value";	$arrv[3] = "'".$in_srcadd."'";
$SQLT = "INSERT INTO ".DB_TBLT." (".join(",", $arrf).") VALUES (".join(",", $arrv).")";
...
if ($afile = fopen($path, "w"))
{
	...
	fprintf($afile, "%s\n", '    <sql3><dbcn>1</dbcn><sql>'.$SQLT.'</sql></sql3>');
	fprintf($afile, "%s\n", '</queries>');
	fclose($afile);
