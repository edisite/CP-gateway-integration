...
log_write($in_trxids, "  Convert Keyword", $in_keywod);
/*
if(strtolower($in_keywod) == "artis")
{
	log_write($in_trxids, "  proses ", "Block");
...
if (DO_CHECK_PARAM)
{
	...
	else
	{
		...
		{
			$SQLCEKBLACK = "SELECT * FROM blacklist_msisdn._msisdn WHERE msisdn = '".$msisdn."' AND (sc = 'all' OR sc = '6768')";
			...
			if($QRYCEKBLACK >= 1)
			{
				...
				log_write($in_trxids, "  error", "Transaction Not sent to operator site\m ");
				echo '<?xml version="1.0"?><push><transid>'.$in_trxids.'</transid><status>0</status><msg>Msisdn blocked</msg></push>';
				return;
			}
		}
	}
...
$SQL4 = "UPDATE ".DB_TBL2." SET sid = '".$in_srvcid."',reply = '".$in_smstxt."',smstype= '".$in_smstyp."', partner ='".$in_operat."', age = '".$in_age."' WHERE trxid = '".$in_trxids."' AND msisdn = '".$in_msisdn."' LIMIT 1";
